...
This release adds preliminary support for the the Sony FX2, FX3/3A, and FX30. The cameras have been validated as fully compatible with CascableCore, and we'll work on adding support for cinema-specific features in future updates. [CORE-1085]
...
- Added support for basic flash settings with `CBLPropertyIdentifierFlashAvailable` (only available on Nikon cameras), `CBLPropertyIdentifierFlashExposureCompensation`, `CBLPropertyIdentifierFlashMode`, and `CBLPropertyCommonValueFlashMode`. [CORE-632]
